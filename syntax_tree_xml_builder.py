...
def proc(tokens_list, i):
    try:
        ...
        if not re.search(r'lex:Id', tokens_list[i].get_description()):
            ...
            sys.exit(0)
        xml_tree = "<proc name='" + tokens_list[i].value + "'>\n"
        i += 1
...
def clause(tokens_list, i):  # Оператор
    ...
    try:
        if tokens_list[i].value.lower() == 'write':
            ...
            xml_subtree, i = goto(tokens_list, i)  # оператор goto
            xml_tree += xml_subtree
        elif tokens_list[i].value.lower() == 'cast':
            xml_subtree, i = cast(tokens_list, i)  # оператор cast
            xml_tree += xml_subtree
        elif tokens_list[i].value.lower() == 'while':
            xml_subtree, i = pwhile(tokens_list, i)  # оператор while
            xml_tree += xml_subtree
            ...
            xml_tree += xml_subtree
            return xml_tree, i
        elif tokens_list[i].lexeme == 'Id':
            xml_subtree, i = call(tokens_list, i)  # Составной оператор
            xml_tree += xml_subtree
            return xml_tree, i
        else:
            pass
...
def operand(tokens_list, i):  # Операнд
    ...
    try:
        if tokens_list[i].value in operations:
            ...
            xml_tree += "<op type='int' kind='" + tokens_list[i].value + "'/>\n"
            i += 1
            return xml_tree, i
        elif re.search(r'lex:TypeReal', tokens_list[i].get_description()):
            xml_tree += "<op type='real' kind='" + tokens_list[i].value + "'/>\n"
            i += 1
            return xml_tree, i
        else:
...
def call(tokens_list, i):
    xml_tree = ''
    try:
        if re.search(r'lex:Id', tokens_list[i].get_description()):
            xml_tree += "<call name='" + tokens_list[i].value + "'/>\n"
            i += 1
        if tokens_list[i].value == '(':
            i += 1
            if re.search(r'lex:Id', tokens_list[i].get_description()):
            ...
            if tokens_list[i].value != ')':
                print('Error:' + str(tokens_list[i].line_number) + ':Ожидается ")"')
            i+=1
        else:
            print('Error:' + str(tokens_list[i].line_number) + ':Ожидается "("')
